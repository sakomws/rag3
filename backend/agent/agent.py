...
class ComplianceHero:
    @classmethod
    async def create(cls, ctx: agents.JobContext):
        agent = ComplianceHero(ctx)
        await agent.start()
    ...
    async def start(self):
        ...
        self.ctx.create_task(
            self.chat.send_message(
                "Welcome to the Comliance Hero Assistant! Speak with me anything related to your SOC-2."
            )
        )
    ...
    async def text_generation_worker(self):
        ...
        while True:
            ...
            if prompt:
                ...
                self.ctx.create_task(
                    self.chat.send_message(
                        f'Loading Compliance Hero for: "{prompt}"'
                    )
                )
                ...
                try:
                   ...
                   if search_string in prompt.lower():
                          ...
                          print(response)
                          self.ctx.create_task(
                            self.chat.send_message(f'Generating evidence to below vendor compliance request: "{reply}"')
                        )
                   else:
                        self.ctx.create_task(
...
if __name__ == "__main__":
    ...
    async def job_request_cb(job_request: agents.JobRequest):
        await job_request.accept(
            ComplianceHero.create,
            identity="Compliance Hero",
